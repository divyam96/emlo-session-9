...
def _load_checkpoint(model, optimizer, args):
    ...
    print("Loading Checkpoint From: {}".format(args.checkpoint_path + '/final.ckpt'))
    checkpoint = torch.load(args.checkpoint_path + '/final.ckpt')
...
def model_fn(model_dir):
    print('model_fn')
    model = CIFAR100_Resnet.load_from_checkpoint(os.path.join(model_dir ,'model.pth'))
    model.eval()
    return model
...
def _train_ligthning(args):
    ...
    trainer.save_checkpoint(os.path.join(args.model_dir ,'model.pth'))
